...
from robot.robotspecifications import ROBOT_FOOT_RADIUS, ROBOT_MAX_SLOPE, ROBOT_SPHERE_RADIUS, ROBOT_FOOT_HEIGHT
from mathtools.linalg import distanceWalkableSurfaceHyperplane, distancePolytopePolytope, getRotationMatrixAligningHyperplaneAndXYPlane
from mathtools.linalg import projectPointOntoHyperplane
from mathtools.linalg import distancePointHyperplane
from mathtools.linalg import sortVertices2D
from mathtools.linalg import getMeanFromVerticesNumpy
from mathtools.linalg import distancePointWalkableSurface
from mathtools.linalg import projectPointOntoWalkableSurface
                        ...
                        for j in range(i+1,len(p.A)):
                                ...
                                if fabs(np.dot(ai,aj)) > 0.99:
                                        ...
                                        if fabs(np.dot(ai,z)) < 0.01 and fabs(np.dot(aj,z)) < 0.01:
                                                ...
                                                if d>0.1:
                                                        vm = vj + 0.5*(vi-vj)
                                                        vni = (vi-vj)/np.linalg.norm(vi-vj)
                                                        vnj = (vj-vi)/np.linalg.norm(vj-vi)

                                                        ### vj <--vnj-- vm --vni--> vi
                                                        vinew = vm+hboxSize*vni
                                                        vjnew = vm+hboxSize*vnj

                                                        p.b[i] = np.dot(vinew, ai)
                                                        p.b[j] = np.dot(vjnew, aj)
